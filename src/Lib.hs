...
generateRandomSuper ::  StdGen -> Bool
generateRandomSuper a = if (value == 1)
                        then True
                        else False
...
setPawnsRevpOne m p l = init $ swap p (length l) tempList
                        where movedPawn = zipWithPair (-) (l !! p) m
                              tempList = l ++ [movedPawn]

setPawnsRevpTwo :: (Integer, Integer) -> Int -> [(Integer, Integer)] -> [(Integer, Integer)]
setPawnsRevpTwo m p l = init $ swap p (length l) tempList
                        where movedPawn = zipWithPair (+) (l !! p) m
                              tempList = l ++ [movedPawn]

moveRecursionRev :: Int -> Int -> Board -> [Board]
moveRecursionRev m p b
  | m >= (length $ moves $ (cards b !! 4)) = []
  | (turn b == 0) && (checkPossibleMove m p b) = [Board {cards = setCardsRev 0 (cards b), pOnePawns = setPawnsRevpOne ((moves $ cards b !! 4)!! m) p (pOnePawns b), pTwoPawns = pTwoPawns b, turn = 1, pOneSuper = pOneSuper b, pTwoSuper = pTwoSuper b, winFlag = False}
        , Board {cards = setCardsRev 1 (cards b), pOnePawns = setPawnsRevpOne ((moves $ cards b !! 4)!! m) p (pOnePawns b), pTwoPawns = pTwoPawns b, turn = 1, pOneSuper = pOneSuper b, pTwoSuper = pTwoSuper b, winFlag = False}
        ] ++ moveRecursionRev (m+1) p b
  | (turn b == 1) && (checkPossibleMove m p b) = [
        Board {cards = setCardsRev 2 (cards b), pOnePawns = pOnePawns b, pTwoPawns = setPawnsRevpTwo ((moves $ cards b !! 4)!! m) p (pTwoPawns b), turn = 0, pOneSuper = pOneSuper b, pTwoSuper = pTwoSuper b, winFlag = False}
        , Board {cards = setCardsRev 3 (cards b), pOnePawns = pOnePawns b, pTwoPawns = setPawnsRevpTwo ((moves $ cards b !! 4)!! m) p (pTwoPawns b), turn = 0, pOneSuper = pOneSuper b, pTwoSuper = pTwoSuper b, winFlag = False}] ++ moveRecursionRev (m+1) p b
  | otherwise = []
...
generateBoard gen = if (winnerGen == 0)
                      ...
                      where winnerGen = generateRandomWinner gen
                            ...
                            pOneSuperGen = generateRandomSuper (snd genF)
                            genG = randomR (0,1::Int) (snd genF)
                            pTwoSuperGen = generateRandomSuper (snd genG)

generateGame :: Integer -> Integer -> String
generateGame seed n = endResult
                      where initGen = mkStdGen $ fromIntegral seed
                            ...
                            result = if (null resultBoardList)
                                     then boardError
                                     else head resultBoardList
                            endResult = if (result == boardError)
                                        then (generateGame reseed n)
                                        else showBoard $ Board {cards = cards result, pOnePawns = [(head $ pOnePawns result)] ++ (sort $ tail $ pOnePawns result), pTwoPawns = [(head $ pTwoPawns result)] ++ (sort $ tail $ pTwoPawns result), turn = mod ((turn result) + 1) 2, pOneSuper = (pOneSuper result), pTwoSuper = (pTwoSuper result), winFlag = False}
                            reseed = toInteger $ fst $ randomR (23, 219382423::Int) initGen
